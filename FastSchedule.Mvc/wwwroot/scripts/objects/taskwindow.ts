class TaskWindow {
  ...
  constructor(year: number, month: number, day: number) {
    ...
    this.Month = month;
    this.Day = day;
    this.ModalWindowHandler = document.querySelector(
  ...
  private async Open(year: number, month: number, day: number) {
    ...
    this.ModalWindowHandler.insertAdjacentHTML(
      "beforeend",
      await AsyncAjaxGet(url)
    );

    document
      .querySelector(".background")
      .setAttribute("style", "-webkit-filter:blur(8px) contrast(70%);");
    ...
    for (const task of this.ModalWindowHandler.querySelectorAll(
      ".update-link"
    )) {
    ...
    let deletedSelectedCompletedDay = document.querySelector(
    ...
    if (deletedSelectedCompletedDay != null) {
      deletedSelectedCompletedDay.addEventListener("click", async function () {
        self.RemoveDay(
          deletedSelectedCompletedDay.id,
          self.Year,
          self.Month,
          self.Day
        );
    ...
    for (const button of this.ModalWindowHandler.querySelectorAll(
      ".delete-button"
    )) {
      let typeChoiseMenu: HTMLElement = button.querySelector(
        ".delete-type-choice"
      );
      let deleteSelectedDayButton = typeChoiseMenu.querySelector(
        ".delete-selected-day"
      );
      let deleteAllTasksButton =
        typeChoiseMenu.querySelector(".delete-all-tasks");
      let closeButton = typeChoiseMenu.querySelector(".close-type-choice");
      closeButton.addEventListener("click", async function () {
        typeChoiseMenu.style.display = "none";
        await setTimeout(function () {
          self.IsDeleteTypeSelectOpen = false;
        }, 100);
      });
    ...
    if (AddTaskDiv != null) {
      this.Task = new Task(
        ...
        0,
        0,
        document.querySelector(".color").id
      );
      ...
      this.ModalWindowHandler.querySelector(".add-button").addEventListener(
        "click",
        function () {
          self.AddTask();
        }
    ...
    let timeInput: HTMLInputElement =
      this.ModalWindowHandler.querySelector("[name='time']");
    let remindBlock: HTMLElement =
      this.ModalWindowHandler.querySelector(".remind");
    if (timeInput != null) {
  ...
  private async OpenUpdateWindow(guid: string) {
    ...
    this.Task = new Task(
      ...
      0,
      0,
      this.ModalWindowHandler.querySelector(".preSelectedColor").id
    );
    let backButton: Element =
      this.ModalWindowHandler.querySelector(".back-button");
    let updateButton: Element =
      this.ModalWindowHandler.querySelector(".update-button");
    ...
    if (this.ValidateData(this.Task)) {
      ...
      if (this.Task.Description != null && this.Task.Description != "") {
        url =
          ...
          this.Task.Label +
          "/" +
          this.Task.ReminderType +
          "/" +
          this.Task.RepeatType +
          "/" +
          this.Task.Color +
          "/" +
  ...
  private async RemoveCompletedDay(
    guid: string,
    year: number,
    month: number,
    day: number
  ...
  private FillTask() {
    this.Task.Label =
      this.ModalWindowHandler.querySelector("[name='label']").value;
    this.Task.Time =
      this.ModalWindowHandler.querySelector("[name='time']").value;
    this.Task.ReminderType =
      this.ModalWindowHandler.querySelector("[name='reminder']").value;
    this.Task.RepeatType =
      this.ModalWindowHandler.querySelector("[name='repeat']").value;
    this.Task.Description = this.ModalWindowHandler.querySelector(
      "[name='description']"
    ).value;
