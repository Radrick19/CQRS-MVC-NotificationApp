class CalendarGrid {
  ...
  constructor(year: number, month: number, day?: number) {
    ...
    oldElementsWithListeners.parentNode.replaceChild(
      newElement,
      oldElementsWithListeners
    );
  ...
  public async OnScrollCheck() {
    if (
      this.maxOpenedMonth < 12 &&
      isLoading == false &&
      this.CalendarHandler.scrollTop >=
        this.CalendarHandler.scrollHeight - window.innerHeight - 150
    ) {
      ...
      isLoading == false &&
      this.CalendarHandler.scrollTop < 400
    ) {
      this.minOpenedMonth -= 1;
  ...
  private async GetStartedMonthes(year: number, month: number, day?: number) {
    ...
    document.querySelector(".total-year").innerHTML =
    ...
    if (day != null) {
      let selectedDay = this.Dates.find(
  ...
  private async AddMonth(year: number, month: number, isOnTop: boolean) {
    ...
    if (isOnTop) {
      ...
      this.CalendarHandler.insertAdjacentHTML(
        "afterbegin",
  ...
  private async SelectDay(grid) {
    if (grid.id != "" && !modalWindowOpened) {
      if (this.SelectedDayGrid != null) {
        this.SelectedDayGrid.querySelector(".manage-grid").style.visibility =
          "hidden";
      ...
      if (
        manageGridButton != null &&
        manageGridButton.getAttribute("listener") !== "true"
      ) {
  ...
  private async UpdateSelectedMonth(
    selectedYear: number,
    selectedMonth: number
  ) {
    let selectedMonthGrids = this.Dates.filter(
      (date) => date.Month == selectedMonth && date.Year == selectedYear
    );
    for await (const oldMonthGrid of document.querySelectorAll(
      ".total-month"
    )) {
